...
async def cmd_craps(message: types.Message):
    ...
    if total in [7, 11]:
        ...
        text += f"🎉 Натуральная победа (Pass Line)! Вы выиграли <b>{profit}</b> сыроежек.{vip_bonus_text}"
        ...
        if rand.choice([True, False]):
            await update_user_balance(chat_id, user_id, profit)
            text += f"🎯 Вы выиграли поинт! Выиграно <b>{profit}</b> сыроежек.{vip_bonus_text}"
        else:
            await update_user_balance(chat_id, user_id, -bet)
